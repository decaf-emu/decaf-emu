...
DisassemblyWindow::draw()
{
   ...
   if (mSelectedAddr != -1) {
      ...
      if (ImGui::IsKeyPressed(static_cast<int>(KeyboardKey::Enter))) {
   ...
   for (auto addr = mAddressScroller.reset(); mAddressScroller.hasMore(); addr = mAddressScroller.advance()) {
      ...
      if (ImGui::IsMouseDoubleClicked(0) && ImGui::IsItemHovered()) {
         if (decaf::debug::hasBreakpoint(addr)) {
            decaf::debug::removeBreakpoint(addr);
         } else {
            decaf::debug::addBreakpoint(addr);
         }
      }
